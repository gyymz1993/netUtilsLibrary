...
public class EncryptUtils {
    ...
    public static EncryptBean transEncryptionParamsRetrofitBase(HashMap<String, ? extends Object> paramsMap, String destination, int priority) {
        ...
        sign = Md5Utils.encode(originSign);
     ...
     */
    public static String decodeResponseToBean(EncryptReturnBean encryptReturnBean) {
        Log.i(TAG, "encryptReturnBean" + encryptReturnBean);
        Log.i(TAG, "onSuccess: check" + Md5Utils.encode(encryptReturnBean.getRandom() + EncryptUtils.getLocalKey()) + "===" + encryptReturnBean.getSign());
        Log.i(TAG, "onSuccess: sign===" + EncryptUtils.getSign() + "===" + encryptReturnBean.getSign());
        if (Md5Utils.encode(encryptReturnBean.getRandom() + EncryptUtils.getLocalKey()).equals(encryptReturnBean.getSign())) {
